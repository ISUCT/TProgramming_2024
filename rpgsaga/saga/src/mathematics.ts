...
export function taskA(xfirst: number, xlast: number, xdelta: number, a: number = 1.2, b: number = 0.48): number[] {
  const result: number[] = [];
  for (let i: number = xfirst; i < xlast; i += xdelta) {
    result.push(formula(i, a, b));
  }
  return result;
}

export function taskB(list: number[], a: number = 1.2, b: number = 0.48): number[] {
  const stack: number[] = [];
